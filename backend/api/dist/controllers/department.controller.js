...
const create = async (req, res, next) => {
    ...
    (0, response_1.sendResponse)(res, 201, 'Department created successfully', result);
};
exports.create = create;
const get = async (req, res, next) => {
    const result = await departmentService.getDepartment(req.params.id);
    (0, response_1.sendResponse)(res, 200, 'Department retrieved', result);
};
exports.get = get;
const getAll = async (req, res, next) => {
    const result = await departmentService.getDepartments(req.user.institutionId);
    (0, response_1.sendResponse)(res, 200, 'Departments retrieved', result);
};
exports.getAll = getAll;
const update = async (req, res, next) => {
    const result = await departmentService.updateDepartment(req.params.id, req.body);
    (0, response_1.sendResponse)(res, 200, 'Department updated', result);
};
exports.update = update;
const remove = async (req, res, next) => {
    const result = await departmentService.deleteDepartment(req.params.id);
    (0, response_1.sendResponse)(res, 200, 'Department deleted', result);
};
