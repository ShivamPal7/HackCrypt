...
const getAttendanceAudit = async (req, res, next) => {
    const result = await auditService.getAttendanceAudit(req.params.id, req.user.institutionId);
    (0, response_1.sendResponse)(res, 200, 'Audit log', result);
};
exports.getAttendanceAudit = getAttendanceAudit;
const getUserAudit = async (req, res, next) => {
    const result = await auditService.getUserAudit(req.params.id, req.user.institutionId);
    (0, response_1.sendResponse)(res, 200, 'User audit', result);
};
