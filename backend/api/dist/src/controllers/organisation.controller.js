...
const organisationService = __importStar(require("../services/organisation.service"));
const response_1 = require("../utils/response");
const create = async (req, res, next) => {
    const result = await organisationService.createOrganisation(req.body, req.user.id);
    (0, response_1.sendResponse)(res, 201, 'Organisation created successfully', result);
};
exports.create = create;
const get = async (req, res, next) => {
    const result = await organisationService.getOrganisation(req.params.id);
    (0, response_1.sendResponse)(res, 200, 'Organisation retrieved', result);
};
exports.get = get;
const update = async (req, res, next) => {
    const result = await organisationService.updateOrganisation(req.params.id, req.body);
    (0, response_1.sendResponse)(res, 200, 'Organisation updated', result);
};
exports.update = update;
const remove = async (req, res, next) => {
    const result = await organisationService.deleteOrganisation(req.params.id);
    (0, response_1.sendResponse)(res, 200, 'Organisation deleted', result);
};
