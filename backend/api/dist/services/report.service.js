...
exports.getStudentLectureHistory = exports.getStudentAttendanceHeatmap = exports.getStudentSubjectDetails = exports.getStudentSubjectSummaries = exports.getStudentAttendanceSummary = exports.getSubjectAttendanceReport = exports.getStudentAttendanceReport = exports.getClassAttendanceReport = void 0;
const prisma_1 = __importDefault(require("../config/prisma"));
const ApiError_1 = __importDefault(require("../utils/ApiError"));
const getClassAttendanceReport = async (classId, institutionId) => {
    const classData = await prisma_1.default.class.findUnique({ where: { id: classId } });
...
const getStudentAttendanceReport = async (studentId, institutionId) => {
...
exports.getStudentAttendanceReport = getStudentAttendanceReport;
const getSubjectAttendanceReport = async (subjectId, institutionId) => {
    // Verify subject
    ...
    const lectures = await prisma_1.default.lecture.findMany({ where: { subjectId }, select: { id: true } });
...
const getStudentAttendanceSummary = async (studentId, institutionId) => {
    ...
    const conductedLectures = await prisma_1.default.lecture.findMany({
        where: {
    ...
    const attendedCount = await prisma_1.default.attendance.count({
        where: {
            studentId,
...
const getStudentSubjectSummaries = async (studentId, institutionId, searchParams) => {
    ...
    const whereSubject = { classId, institutionId };
    ...
    const summaries = await Promise.all(subjects.map(async (subject) => {
        ...
        const totalConducted = await prisma_1.default.lecture.count({
            where: {
        ...
        const totalAttended = await prisma_1.default.attendance.count({
            where: {
                studentId,
                status: { in: ['PRESENT', 'LATE'] },
                lecture: {
...
const getStudentSubjectDetails = async (studentId, subjectId, institutionId) => {
    // 1. Verify Student
    const student = await prisma_1.default.user.findUnique({
        where: { id: studentId },
    });
    ...
    if (student.studentClassId !== subject.classId) {
        throw new ApiError_1.default(403, 'Subject does not belong to your class');
    }
    // 3. Calculate Metrics
    ...
    // Attended (Present)
    const lecturesAttended = await prisma_1.default.attendance.count({
        where: {
            studentId,
    ...
    // Late
    const lecturesMarkedLate = await prisma_1.default.attendance.count({
        where: {
            studentId,
            lectureId: { in: lectureIds },
            status: 'LATE'
...
const getStudentAttendanceHeatmap = async (studentId, institutionId) => {
    // 1. Verify Student
    const student = await prisma_1.default.user.findUnique({
        where: { id: studentId },
    });
...
const getStudentLectureHistory = async (studentId, institutionId, query) => {
    // 1. Verify Student
    const student = await prisma_1.default.user.findUnique({
        where: { id: studentId },
    });
