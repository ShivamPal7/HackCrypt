...
export function VerifyEmailScreen() {
    ...
    const [timeLeft, setTimeLeft] = useState(42); // 42 seconds as per design
            ...
            >
                ...
                >
                    <Animated.View entering={FadeInDown.duration(400)} className="mb-10">
                        <Text className="text-3xl font-inter-bold text-foreground mb-4">
                            Verify Email
                        </Text>
                        <Text className="text-muted-foreground text-base font-inter-medium leading-relaxed">
                            Please enter the 4-digit code sent to your email address
                        </Text>
                        <Text className="text-foreground text-base font-inter-bold mt-1">
                            m.alumni@university.edu
                        </Text>
                    </Animated.View>
                            ...
                            {otp.map((digit, index) => (
                                <View key={index} className="relative">
                                    <TextInput
                                        ...
                                        placeholder="-"
                                        placeholderTextColor="#2A3441" // Darker than standard muted to match "empty" look
                                    />
                                    {/* Cursor simulation if needed, but native cursor is usually fine */}
                                </View>
                            ))}
                        </View>
                        ...
                        <View className="items-center">
                            ...
                            <View className="bg-[#1A2232] px-4 py-2 rounded-full flex-row items-center space-x-2">
                                ...
                                <Text className="text-muted-foreground font-inter-medium text-sm ml-2">
                                    Wait <Text className="text-foreground font-inter-bold">{formatTime(timeLeft)}</Text> to resend
                                </Text>
                            </View>
                        </View>
                    </Animated.View>
